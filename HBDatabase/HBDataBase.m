...
@property (nonatomic, strong) NSMutableArray<HBContentTable *> *tableArray;
...
- (void)updateDB:(FMDatabase *)aDB{
    [self.tableArray enumerateObjectsUsingBlock:^(HBContentTable * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [obj updateDB:aDB];
    }];
}
...
#pragma mark - 简化创建的方法
- (HBContentTable *)registTableClass:(Class)aClass{
    
    HBContentTable *table = [[aClass alloc] init];
    NSAssert([table isKindOfClass:[HBContentTable class]] , @"必须是继承HBContentTable 的类");
...
#pragma mark - 懒加载
- (NSMutableArray<HBContentTable *> *)tableArray{
    if (!_tableArray) {

...
#define KEY_SIGNAL_OBJSTR(TARGETCLSNAME,KEY) [NSString stringWithFormat:@"%@%@%@",TARGETCLSNAME,TARGETCLSNAME.length?KEY_SEPARATED:@"",KEY]
...
{
    [self observerWithObject:object key:key targetclass:targetclass from:nil];
}
...
-(void)observerWithObject:(id)observer key:(NSString *)key targetclass:(NSString *)targetclass from:(id)from
{
    ...
    SEL selector = NSSelectorFromString([NSString stringWithFormat:@"handelSignal_%@_%@:",targetclass,newkey]);

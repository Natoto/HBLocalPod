...
- (void)openURLString:(NSString *)URLString parameters:(NSDictionary *)parameters animation:(BOOL)animate
{
    ...
    isChanged = [self __handleModalWithURL:url params:parameters];
    ...
    if (start>1 && start == components.count) {
        //openurl正好是前面已经有的堆栈则只需要执行pop的队列
...
- (BOOL)__handleModalWithURL:(NSURL *)URL
{
    ...
    if ([@"modal" isEqualToString:scheme] && host.length > 0)
    {
        ...
        [self.rootViewController presentViewController:navi animated:animated completion:nil];
        return YES;
    }
...
// 处理模态视图 hbadd
- (BOOL)__handleModalWithURL:(NSURL *)URL params:(NSDictionary *)params
{
    NSString *scheme = URL.scheme;
    NSString *host   = URL.host;
    ...
    if ([@"modal" isEqualToString:scheme] && host.length > 0)
    {
        ...
        if (components.count < 2)
        {
            NSDictionary *queryDictonary = params;//[self __dictionaryFromQuery:URL.query];
            [queryDictonary enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
                // todo 安全性检查
                [vc setValue:obj forKey:key];
            }];
...
- (void)__pushViewController:(UIViewController *)viewController parameters:(NSDictionary *)parameters atNavigationController:(UINavigationController *)navigationController animated:(BOOL)animated
{
    ...
    [parameters enumerateKeysAndObjectsUsingBlock:^(id akey, id obj, BOOL *stop) {
        // TODO: 安全性检查
        NSString * key = [akey stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];;

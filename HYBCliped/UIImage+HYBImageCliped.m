...
static const char *s_hyb_image_notshowinnercircle = "s_hyb_image_notshowinnercircle";

@implementation UIImage (HYBImageCliped)

#pragma mark - Border
- (BOOL)hyb_notshowinnercircle {
    NSNumber *borderWidth = objc_getAssociatedObject(self, s_hyb_image_notshowinnercircle);
    
    if ([borderWidth respondsToSelector:@selector(boolValue)]) {
        return borderWidth.boolValue;
    }
    
    return 0;
}

- (void)setHyb_notshowinnercircle:(BOOL)hyb_notshowinnercircle {
    objc_setAssociatedObject(self,
                             s_hyb_image_notshowinnercircle,
                             @(hyb_notshowinnercircle),
...
- (UIImage *)hyb_private_clipImageToSize:(CGSize)targetSize
  ...
  if (pathWidth > 0 && borderWidth > 0 && (isCircle || cornerRadius == 0)) {
    ...
    CGFloat minusPath1 = pathWidth / 2;
      if (self.hyb_notshowinnercircle == NO) {
          CGContextSetStrokeColorWithColor(ctx, [borderColor CGColor]);
          CGContextSetLineWidth(ctx, borderWidth);

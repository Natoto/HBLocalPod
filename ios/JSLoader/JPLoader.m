...
-(NSString *)rootUrl{
    if (!_rootUrl) {
        NSAssert(_rootUrl, @"jspatch脚本下发链接 rooturl 为空");
    }
    return _rootUrl;
}

-(NSString *)patchVersionName{
    if (!_patchVersionName) {
        _patchVersionName = @"patchVersion.json";
        NSLog(@"脚本文件未设置，默认为 patchVersion.json");
    }
    return _patchVersionName;
}
...
+ (void)updateToVersion:(NSInteger)version ex:(NSString *)ex callback:(JPUpdateCallback)callback{
    ...
    if (ex.length) {
        ...
        downloadURL = [NSURL URLWithString:[[JPloaderData sharedInstance].rootUrl stringByAppendingString:downloadKey]];
    }

...
#import "HBClearCacheTool.h"

#define fileManager [NSFileManager defaultManager]

@implementation HBClearCacheTool
...
+ (BOOL)clearCacheWithFilePath:(NSString *)path
{
    ...
    NSString *filePath = nil;
    __block BOOL valiate = YES;
    dispatch_queue_t _queue = dispatch_queue_create("com.olinone.synchronize.serialQueue", NULL);
    ...
    for (NSString *subpath in subpathArray)
    {
        dispatch_async(_queue, ^{
            ...
            NSString* message1;
            NSString* filePath1 =[path stringByAppendingPathComponent:subpath];
            //删除子文件夹
            [fileManager removeItemAtPath:filePath1 error:&error];
            
            if (error) {//&& ![[HBClearCacheTool whiteMembers] containsObject:subpath]
                message1 = [NSString stringWithFormat:@"%@这个路径的文件夹删除失败了，请检查后重新再试",filePath1];
                valiate = NO;
                NSLog(@"%@",message);
            }else {
                message1 = @"成功了";
...
//+(NSArray *)whiteMembers{//白名单这些不删除
//    return @[@"Snapshots",@"com.apple.WebKit.Networking",@"com.apple.WebKit.WebContent",@"WBSDKStatistics.db",@"LaunchImages"];
//}

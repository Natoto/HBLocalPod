...
-(void) configureScrollView
{
    ...
    {
        if ([_dataSource respondsToSelector:@selector(FeSlideFilterView:viewForTitleAtIndex:)]) {
            UIView * view =  [_dataSource FeSlideFilterView:self viewForTitleAtIndex:i];
            if (view) {
                CGRect oldframe = view.frame;
                CGRect newframe = CGRectMake(oldframe.origin.x + frame.size.width * i, oldframe.origin.y + 0, oldframe.size.width, oldframe.size.height);
                view.frame = newframe;
        ...
        // Title
        NSString * title ;
        if ([_dataSource respondsToSelector:@selector(FeSlideFilterView:titleFilterAtIndex:)]) {
            title = [_dataSource FeSlideFilterView:self titleFilterAtIndex:i];
        }
